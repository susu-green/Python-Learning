...
**代码如下：**
    ...
    def getText():
        ...
        for ch in '!"#<img src="https://latex.codecogs.com/gif.latex?%&amp;()*+,-.&#x2F;:;&lt;=&gt;?@[&#x5C;&#x5C;]^_‘{|}~&#x27;:#将各种特殊字符替换为空格%20%20%20%20%20%20%20%20#整个文本视为一个长字符串，便于后续的字符串切片%20%20%20%20%20%20%20%20%20%20%20%20txt=txt.replace(ch,&quot;%20&quot;)%20%20%20%20%20%20%20%20return%20txt%20%20%20%20if%20__name__%20==&#x27;__main__&#x27;:%20%20%20%20%20%20%20%20Txt=getText()%20%20%20%20%20%20%20%20words=Txt.split()%20%20%20%20%20%20%20%20cnt%20=%200%20%20%20%20%20%20%20%20for%20word%20in%20words:%20%20%20%20%20%20%20%20%20%20%20%20cnt%20=%20cnt%20+%201%20%20%20%20%20%20%20%20print(cnt)----------**第%200005%20题：**%20你有一个目录，装了很多照片，把它们的尺寸变成都不大于%20iPhone5%20分辨率的大小。**大概思路：**iPhone%205%20的分辨率为%201136"/>\times<img src="https://latex.codecogs.com/gif.latex?640.通过OS库中的listdir()方法列出所有的文件，并且通过os.path.splitext(path)分离文件名和扩展名，筛选出扩展名为**&#x27;jpg&#x27;**的文件。对选出的文件进行尺寸修改，最后保存到设定好的文件位置。**背景资料：**[os.path模块][11][Python3%20os.listdir()%20方法][12]&gt;%201.os.path.abspath(path)&gt;%20返回path规范化的绝对路径。&gt;%202.os.path.split(path)&gt;%20&gt;%20将path分割成目录和文件名二元组返回。&gt;%20&gt;%203.os.path.dirname(path)&gt;%20&gt;%20返回path的目录。其实就是os.path.split(path)的第一个元素。&gt;%20&gt;%204.os.path.basename(path)&gt;%20&gt;%20返回path最后的文件名。如何path以／或&#x5C;结尾，那么就会返回空值。即os.path.split(path)的第二个元素。&gt;%20&gt;%205.os.path.commonprefix(list)&gt;%20&gt;%20返回list中，所有path共有的最长的路径。&gt;%20&gt;%206.os.path.exists(path)&gt;%20&gt;%20如果path存在，返回True；如果path不存在，返回False。&gt;%20&gt;%207.os.path.isabs(path)&gt;%20&gt;%20如果path是绝对路径，返回True。&gt;%20&gt;%208.os.path.isfile(path)&gt;%20&gt;%20如果path是一个存在的文件，返回True。否则返回False。&gt;%20&gt;%209.os.path.isdir(path)&gt;%20&gt;%20如果path是一个存在的目录，则返回True。否则返回False。&gt;%20&gt;%2010.os.path.join(path1[,%20path2[,%20...]])&gt;%20&gt;%20将多个路径组合后返回，第一个绝对路径之前的参数将被忽略。&gt;%20&gt;%2011.os.path.normcase(path)&gt;%20&gt;%20在Linux和Mac平台上，该函数会原样返回path，在windows平台上会将路径中所有字符转换为小写，并将所有斜杠转换为反斜杠。&gt;%20&gt;%2012.os.path.normpath(path)&gt;%20&gt;%20规范化路径。&gt;%2013.os.path.splitdrive(path)&gt;%20&gt;%20返回（drivername，fpath）元组&gt;%20&gt;%2014.os.path.splitext(path)&gt;%20&gt;%20分离文件名与扩展名；默认返回(fname,fextension)元组，可做分片操作&gt;%20&gt;%2015.os.path.getsize(path)&gt;%20&gt;%20返回path的文件的大小（字节）。&gt;%2016.os.path.getatime(path)&gt;%20&gt;%20返回path所指向的文件或者目录的最后存取时间。&gt;%20&gt;%2017.os.path.getmtime(path)&gt;%20&gt;%20返回path所指向的文件或者目录的最后修改时间[Python%20PIL%20Image.save()用法及代码示例][13]**问题分析：**[*OSError:%20cannot%20write%20mode%20RGBA%20as%20JPEG的解决办法*][14][Python%20PIL:%20cannot%20write%20mode%20RGBA%20as%20BMP][15]**PNG有RGBA四个通道，而JPEG是RGB三个通道，所以PNG转BMP时候程序不知道A通道怎么办，就会产生错误。解决方法就是检查通道数，舍弃A通道。**代码如下：%20%20%20%20import%20os%20%20%20%20from%20PIL%20import%20Image%20%20%20%20%20%20%20%20def%20handle_photo(dir1,dir2):%20%20%20%20%20%20%20%20list_file%20=%20os.listdir(dir1)%20%20%20%20%20%20%20%20for%20file%20in%20list_file:%20%20%20%20%20%20%20%20%20%20%20%20if%20os.path.splitext(file)[1]%20==%20&#x27;.jpg&#x27;:%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20image%20=%20Image.open(dir1%20+%20&#x27;&#x2F;&#x27;+file)%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20image%20=%20image.convert(&quot;RGB&quot;)#需要注意：%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20image.thumbnail((1160,640))%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20image.save(dir2%20+%20&#x27;&#x2F;&#x27;%20+%20file)%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20print(&quot;已经过处理的图片：&quot;,file)%20%20%20%20if%20__name__%20==&quot;__main__&quot;:%20%20%20%20%20%20%20%20dir1%20=%20&quot;xxx&#x2F;test1&quot;%20%20%20%20%20%20%20%20dir2%20=%20&quot;xxx&#x2F;test2&quot;%20%20%20%20%20%20%20%20handle_photo(dir1,dir2)----------**第%200006%20题：**%20你有一个目录，放了你一个月的日记，都是%20txt，为了避免分词的问题，假设内容都是英文，请统计出你认为每篇日记最重要的词。**大概思路：**通过OS库中的listdir()方法列出所有的文件，读取纯文本文件，将其中的各个特殊字符提换为空格，将整个文本视为一个长字符串，再对它进行切片。统计切片后的字符串（单个单词）个数，同时利用字典数据结构，遍历字符串的时候相同的单词如果出现，键值就会加1.最后用items()方法，将字典中的每对元素组成一个元组并且在列表中返回，对元素中的第二个字段(单词出现的次数)进行逆序排序，输出第一个元素的第一个字段，便是出现次数最多的元素。**资料背景：**第0004题和0005题的结合，具体可参考第0004题和0005题和**问题分析**。**问题分析：**%20-%20[&#x27;gbk&#x27;%20codec%20can&#x27;t%20decode%20byte%200x80%20in%20position%202:%20illegal%20multibyte%20sequence][16]%20编码格式的原因，改成&quot;encoding=&#x27;utf-8&quot;。%20-%20[Python%20中的%20&quot;%s&quot;%用法][17]%20-%20[items.sort()][18]%20-%20sort()函数用于对列表进行排序，所以要转换成列表数据结构。%20**代码如下：**%20%20%20%20import%20os%20%20%20%20%20%20%20%20def%20getText(dir,file):%20%20%20%20%20%20%20%20txt%20=%20open(dir%20+&#x27;&#x2F;&#x27;%20+%20file,%20&quot;r&quot;,encoding=&#x27;utf-8&#x27;).read()%20%20%20%20%20%20%20%20txt=txt.lower()%20#默认单词的大小写不同，但都是一个单词%20%20%20%20%20%20%20%20for%20ch%20in%20&#x27;!&quot;#"/>%&()*+,-./:;<=>?@[\\]^_‘{|}~':
    ...
    def count_words_txt(dir,file):
        ...
        for word in words: #遍历切片后产生的字符段
         #字典中存的数据类型为{key : value},key 为word. value为 单词出现的次数
            counts[word]=counts.get(word,0)+1
            # items() 方法把字典中每对 key 和 value 组成一个元组，
            #并把这些元组放在列表中返回。
